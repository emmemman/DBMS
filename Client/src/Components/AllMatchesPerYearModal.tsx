...
import { useGetAllMatchesPerYearWithDetailsQuery } from "../../../Shared/generated/graphql";
...
export const AllMatchesPerYearModal = (props: IProps) => {
  const { data, loading } = useGetAllMatchesPerYearWithDetailsQuery({
    variables: {
      year: props.year,
    },
    skip: !props.year,
  ...
  const rawData = data?.getAllMatchesPerYearWithDetails?.matches || [];
  ...
  const filteredData = rawData
    ?.filter((match) => {
      const matchesDeveloped =
        ...
        (match?.homeTeam?.developedStatus === selectedDeveloped &&
          match?.awayTeam?.developedStatus === selectedDeveloped);

      const matchesStatus =
        selectedStatus === "ALL" ||
        (match?.homeTeam?.status === selectedStatus &&
          match?.awayTeam?.status === selectedStatus);

      const matchesRegion =
        selectedRegion === "ALL" ||
        (match?.homeTeam?.regionName === selectedRegion &&
